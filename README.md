# (Sync-Async) CompositeOperations 

**SACompositeOperations = NSOperation/NSOperationQueue + block-based DSL**
 
`SACompositeOperations` is an attempt to build a higher-level composite operations framework on top of GCD. Its main features are:
...
[![Build Status](https://travis-ci.org/stanislaw/SACompositeOperations.png?branch=master)](https://travis-ci.org/stanislaw/SACompositeOperations)
...
`SATransactionalOperation` due to its async- nature does not block its original queue/thread (it just schedules sub-operations blocks there) but provides a completion handler instead, which is called when all sub-operations are over.
...
__Suboperations__ Both cascade and transactional operations schedule and run suboperations declared inside their bodies. Each suboperation block generates SAOperation class that runs sub-operation block assigned to it and also stores its parent context (cascade- or transactional-) to control parent operation's flow. Important: suboperations should not be run or touched standalone, but only from inside their corresponding cascade- or transactional- operational contexts only!
...
The purpose of this code is to abstract the logic of these operations: two single operations (`SAOperation` and `SASyncOperation`) and two complex (or "composite" which is better) operations (`SACascadeOperation` and `SATransactionalOperation`).
...
Just add `SACompositeOperations/` folder to your project.

Or add into your Podfile:

```ruby
pod 'SACompositeOperations', :git => 'https://github.com/stanislaw/SACompositeOperations'
pod update
```
...
#import <SAQueues.h>
// ...
[SAQueues setDefaultQueue:dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)];
```

Then import `SACompositeOperations.h` where you want to use SA operations.
...
The `SAOperation` is just a block of code wrapped into an NSOperation subclass with additional capabilities: since SAOperation yields itself to the block to be executed (see `Each operation yields itself to the block`), it is possible to get more control of operation's execution flow: it is possible to cancel, suspend/resume, rerun (and some more options) operation from inside its own operation body.

The most simple example: use `operation` (C-function wrapper for SAOperation, part of SACompositeOperations DSL) to create and immediatedly run an operation block:

```objective-c
operation(concurrentQueue(), ^(SAOperation *operation) {
    /*
...
### SATransactionalOperation

The following example will run transactional operation - it will schedule 3 asynchronous suboperations (All 3 operations will be run in a queue set by SASetDefaultQueue()).

The order of execution:

1. The outer transactional operation block - schedules suboperations.
2. All 3 suboperations will be run in an order dictated by the SA default queue (serial or concurrent - both are fine)
...
transactionalOperation(^(SATransactionalOperation *transaction) {
    [transaction operation:^(SAOperation *operation) {
        /* 
          ...
          since SADefaultQueue() is concurrent...
    ...
    */
    ...
    isFinished = YES;
}, ^(SATransactionalOperation *transaction){
    /*
      ...Cancellation handler code...
      ...called immediatedly if at least one of suboperations is cancelled.
      ...here maybe resolve transaction somehow: rerun, rerun after, or cancel... see <SAOperationResolver>
    */
});
...
cascadeOperation(^(SACascadeOperation *cascade){
    [cascade operationInQueue:queue1 operation:^(SAOperation *operation){
        /* 
          ...first job... 
        */
    ...
    [cascade operationInQueue:queue2 operation:^(SAOperation *operation){
        /* 
          ...second job... 
        */
    ...
    [cascade operationInQueue:queue3 operation:^(SAOperation *operation){
        /* 
          ...third job... 
        */
    ...
    [cascade operationInQueue:queue4 operation:^(SAOperation *operation){
        /* 
          ...fourth job... 
        */
...
The following will run cascade operation - it will schedule 3 asynchronous suboperations (All 3 operations will be run in a queue set by SASetDefaultQueue()).
...
cascadeOperation(^(SACascadeOperation *co) {
    ...
    // At this point - after the outer block have just run - all operations are scheduled to be run in SADefaultQueue()
}, nil, nil);
...
or using helper functions defined in the global namespace in `SACompositeOperations.h`:
...
Non-composite operations: `SAOperation` and `SASyncOperation` always must be finished somewhere inside their operation blocks. 
